...
// ***place file for isReferenced() here***

/**
 * Represents an organization, which is an organization that has a Meteor account.
 *
 * Organizations are defined when the various OrganizationProfile collections are initialized, so the Organization collection is the union
 * of Organizations, plus the single Admin account who also has a Meteor account.
 *
...
class OrganizationCollection {
  ...
  /**
   * Define a new organization, which means creating an entry in Meteor.Accounts.
   * This is called in the various OrganizationProfile define() methods.
  ...
  /**
   * Asserts that the passed organization has the specified role.
  ...
  // removed isReferenced(), ***put here if needed***

  /**
   * Returns true if organization is a defined userID or username.
   * @param user The organization.
   * @returns { boolean } True if organization is defined, false otherwise.
   */
  isDefined(user) {
  ...
  /**
   * Returns the userID associated with organization, or throws an error if not defined.
   * @param user The organization (username or userID).
   * @returns { String } The userID
   * @throws { Meteor.Error } If organization is not a defined username or userID.
   */
  getID(user) {
    const userDoc = (Meteor.users.findOne({ _id: user })) || (Meteor.users.findOne({ username: user }));
    if (!userDoc) {
  ...
  /**
   * Returns the userIDs associated with organizations, or throws an error if any cannot be found.
   * @param { String[] } users An array of valid organizations.
   * @returns { String[] } The docIDs associated with organizations.
   * @throws { Meteor.Error } If any instance is not a organization.
   */
  getIDs(users) {
    let ids;
    try {
  ...
  /**
   * Returns the profile document associated with organization.
