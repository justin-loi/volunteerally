...
class BaseProfileCollection extends BaseCollection {
   ...
   */
  getProfile(user) {
    const userID = Volunteers.getID(user);
  ...
  removeIt(profileID) {
    ...
    // use if volunteers have some related collection.
    // if (!Volunteers.isReferenced(userID)) {
    Meteor.users.remove({ _id: userID });
    return super.removeIt(profileID);
    // }
    // throw new Meteor.Error(`User ${profile.email} owns {thisCollection}.`);
  }
